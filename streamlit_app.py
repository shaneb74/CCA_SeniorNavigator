...
# Robust currency parsing: accepts $, commas, decimals, (1,200.35) negatives
def parse_currency_str(s, default=0.0):
    ...
    try:
        return sign * float(s) if s else float(default)
    except Exception:
        return float(default)

# ---------------- Spec & Lookups ----------------
def load_spec():
    spec = read_json("senior_care_calculator_v5_full_with_instructions_ui.json")
    ov = read_json("senior_care_modular_overlay.json")
    if ov:
        spec.setdefault("lookups", {}).update(ov.get("lookups", {}))
    spec.setdefault("lookups", {})
    L = spec["lookups"]
    L.setdefault("state_multipliers", {"National": 1.0})
    L.setdefault("room_type", {"Studio": 3500, "1 Bedroom": 4200, "Shared": 3000})
    L.setdefault("care_level_adders", {"Low": 200, "Medium": 600, "High": 1200})
    L.setdefault("mobility_adders", {
        "facility": {"No support needed": 0, "Walker": 150, "Wheelchair": 300},
        "in_home": {"Low": 0, "Medium": 10, "High": 20},
    })
    L.setdefault("chronic_adders", {"None": 0, "Some": 150, "Multiple/Complex": 400})
    L.setdefault("in_home_care_matrix", {2: 120, 4: 220, 6: 300, 8: 380, 10: 450})
    # Monthly ceilings approximation (MAPR / 12) – replace with latest when needed
    L.setdefault("va_categories", {
        "None": 0.0,
    ...
    S = spec["settings"]
    S.setdefault("memory_care_multiplier", 1.25)
    S.setdefault("second_person_cost", 1200.0)
    S.setdefault("display_cap_years_funded", 30)
    return spec
...
def mark_touched(name=None):
    ensure_touched_store()
    if name:
        st.session_state.drawer_touched[name] = True
        st.session_state.open_drawers[name] = True
    # flip a flag to force a rerun
    st.session_state['__dirty_flag'] = not st.session_state.get('__dirty_flag', False)
...
# Currency input with $0.00 placeholder; selects on focus; commits on blur via on_change
def currency_input(label, store_name, default=0.0, drawer_name=None, help_text=None):
    ...
    if raw_key not in st.session_state:
        st.session_state[raw_key] = (f"{existing:,.2f}" if existing not in (None, "", 0, 0.0) else "")
    raw = st.text_input(
        label,
...
def compute(inputs, spec):
    ...
    chronic = L["chronic_adders"]; mat = L["in_home_care_matrix"]
    ...
    hh = sum(float(inputs.get(k, 0.0)) for k in [
        "ss_a","pension_a","ss_b","pension_b",
        "disability","rental_income","wages_part_time","alimony_support",
        "dividends_interest","other_income_monthly"
    ])
    # LTC benefits if toggled
    ...
    if "Two veterans" in catA or "Two veterans" in catB:
        ...
        va_a = va_month; va_b = 0.0
    elif "Veteran" in catB or "spouse" in catB:
        va_b = va_month; va_a = 0.0
    else:
        va_a = 0.0; va_b = 0.0
...
def home_mods_ui(inp):
    ...
    with expander(name, "Home modifications (one-time costs)", total):
        ...
        # Friendly warning if neither has In-Home care
        ...
        def item(key, label, hint, low, high, avg):
            ...
            if spec_choice == "Typical":
                ...
                st.info(f"Typical install: {mfmt(avg)}\u00A0•\u00A0Range {mfmt(low)}–{mfmt(high)}.")
                inp[f"hm_{key}_val"] = val
                mark_touched(name)
            elif spec_choice == "Basic":
                st.info(f"Basic estimate set to {mfmt(low)}\u00A0•\u00A0Range {mfmt(low)}–{mfmt(high)}.")
                inp[f"hm_{key}_val"] = float(low)
                val = float(low)
                mark_touched(name)
            else:
                val = st.slider(f"Custom estimate — {label}", int(low), int(high), int(inp.get(f"hm_{key}_val", avg) or avg), 25, key=f"hm_{key}_slider", on_change=mark_touched, args=(name,))
                inp[f"hm_{key}_val"] = float(val)
                st.caption(f"Typical: {mfmt(avg)}\u00A0•\u00A0Range {mfmt(low)}–{mfmt(high)}.")
            st.caption(hint)
...
def main():
    ...
    # -------- Step 2 --------
        ...
        def person(tag, display, want_default_stay=False):
            ...
            ct = st.selectbox(f"Care type for {display}", ALL_CT, key=f"ct_{tag}", help="Change this to compare options. In‑Home shows sliders; facilities show room/level options.")
            ...
            if ct.startswith("In-Home"):
                ...
                room = st.selectbox("Room type", list(spec["lookups"]["room_type"].keys()), index=0, key=f"room_{tag}", help="Common choices; costs vary by state and community.")
            ...
            if ct == "Stay at Home (no paid care)":
                ...
                lvl = st.selectbox("Care level", ["Low (help with a few tasks)", "Medium (daily support with several tasks)", "High (extensive supervision and care)"], index=1, key=f"lvl_{tag}", help="Use your best judgment; this adjusts estimated cost bands.")
                inp[f"care_level_{tag}"] = lvl.split(" (")[0]
                mob = st.selectbox("Mobility", ["No support needed (independent)", "Walker (needs walker or cane)", "Wheelchair (primarily wheelchair)"], index=1, key=f"mob_{tag}", help="Mobility needs drive staff time and equipment, which affect cost.")
                inp[f"mobility_{tag}"] = mob.split(" (")[0]
                cc = st.selectbox("Chronic conditions", ["None (no chronic conditions)", "Some (one or two managed)", "Multiple/Complex (multiple or complex care)"], index=0, key=f"cc_{tag}", help="More complex conditions generally increase care intensity.")
    ...
    # -------- Step 3 --------
        ...
        # Income A (live drawer header from _raw)
        income_a_preview = (
            parse_currency_str(st.session_state.get("ss_a_raw", ""), 0.0)
            + parse_currency_str(st.session_state.get("pension_a_raw", ""), 0.0)
        )
        with expander("income_a", f"Income — {names.get('A','Person A')}", income_a_preview):
            inp["ss_a"] = currency_input("Social Security (monthly)", "ss_a", default=inp.get("ss_a", 0.0), drawer_name="income_a", help_text="Type a number like 1,800 or 1,800.50; commas and $ are fine.")
        ...
        if st.session_state.get("include_b", False):
            income_b_preview = (
                parse_currency_str(st.session_state.get("ss_b_raw", ""), 0.0)
                + parse_currency_str(st.session_state.get("pension_b_raw", ""), 0.0)
            )
        ...
        with expander("benefits", "Benefits — VA Aid & Attendance, Long‑Term Care insurance, and other supports.", float(va_preview["va_a"]) + float(va_preview["va_b"]) + float(inp.get("ltc_a_monthly", 0.0)) + float(inp.get("ltc_b_monthly", 0.0))):
            ...
            with c1:
                sel_a = st.selectbox(
                    f"VA category — {names.get('A','Person A')}",
                    [catdisplay(c) for c in cats],
                    index=0,
                    key="va_cat_a_key",
                    help="Pick the VA eligibility category. This sets the household ceiling (MAPR).",
                    on_change=mark_touched, args=("benefits",),
                )
                inp["va_cat_a"] = sel_a.split(" (")[0]
            if st.session_state.get("include_b", False):
                with c2:
                    sel_b = st.selectbox(
                        f"VA category — {names.get('B','Person B')}",
                        [catdisplay(c) for c in cats],
                        index=0,
                        key="va_cat_b_key",
                        help="If both are veterans with A&A, choose the 'Two veterans' ceiling.",
                        on_change=mark_touched, args=("benefits",),
                    )
                    inp["va_cat_b"] = sel_b.split(" (")[0]

            st.caption("**How this works:** The VA category sets the MAPR (ceiling). We compute countable income and subtract deductible medical (care + Medicare + dental/vision/hearing + prescriptions + personal care). Award = max(0, MAPR − max(0, income − medical)) ÷ 12. If 'Two veterans', the award is split between A and B. You can override to match an award letter.")
        ...
        with expander("other_costs", "Other monthly costs (optional)", other_preview):
            inp["medicare"] = currency_input("Medicare premiums", "medicare", default=inp.get("medicare", 0.0), drawer_name="other_costs", help_text="Part B + supplemental plans.")
        ...
        with expander("assets_common", "Assets — Common balances", assets_common_preview):
            ...
            inp["home_equity"] = currency_input("Home equity (net of payoff)", "home_equity", default=inp.get("home_equity", 0.0), drawer_name="assets_common")
        ...
        # Home equity & housing options (moved from Step 1)
        he_preview = float(inp.get("home_equity", 0.0))
        with expander("home_equity", "Home equity & housing options", he_preview):
            choice = st.radio(
                "How should we treat the home?",
                [
                    "Keep the home (don't tap equity)",
                    "Sell the home (use net proceeds)",
                    "Use reverse mortgage (HECM)",
                    "Consider a HELOC (home equity line)",
                ],
                index=0 if not inp.get("home_to_assets") and not inp.get("expect_hecm") and not inp.get("expect_heloc") else (1 if inp.get("home_to_assets") else (2 if inp.get("expect_hecm") else 3)),
                key="home_plan_step3",
                help="Choose how the home fits the plan. 'Sell' moves net proceeds to Assets; HECM/HELOC draws can be added below.",
                on_change=mark_touched, args=("home_equity",),
            )
            inp["maintain_home_household"] = choice.startswith("Keep")
            inp["home_to_assets"] = choice.startswith("Sell")
            inp["expect_hecm"] = "HECM" in choice
            inp["expect_heloc"] = "HELOC" in choice

            if inp["home_to_assets"]:
                st.subheader("Home sale estimate")
                c1, c2, c3 = st.columns(3)
                with c1:
                    sell = currency_input("Estimated sale price", "sell_price", default=inp.get("sell_price", 0.0), drawer_name="home_equity")
                with c2:
                    payoff = currency_input("Est. mortgage payoff", "mortgage_payoff", default=inp.get("mortgage_payoff", 0.0), drawer_name="home_equity")
                with c3:
                    fees = currency_input("Selling costs (fees, repairs, etc.)", "selling_fees", default=inp.get("selling_fees", 0.0), drawer_name="home_equity")
            ...
            if inp.get("maintain_home_household"):
                st.caption("These carrying costs apply only while someone is still living in the home.")
                c1, c2, c3 = st.columns(3)
                with c1:
                    inp["mortgage"] = currency_input("Monthly mortgage", "mortgage", default=inp.get("mortgage", 0.0), drawer_name="home_equity")
                    inp["insurance"] = currency_input("Homeowner’s insurance (monthly)", "insurance", default=inp.get("insurance", 0.0), drawer_name="home_equity")
                with c2:
                    inp["taxes"] = currency_input("Property taxes (monthly)", "taxes", default=inp.get("taxes", 0.0), drawer_name="home_equity")
                    inp["hoa"] = currency_input("HOA fees (monthly)", "hoa", default=inp.get("hoa", 0.0), drawer_name="home_equity")
                with c3:
                    inp["utilities"] = currency_input("Utilities (monthly)", "utilities", default=inp.get("utilities", 0.0), drawer_name="home_equity")

            if inp.get("expect_hecm"):
                inp["hecm_draw"] = currency_input("HECM (reverse mortgage) monthly draw (optional)", "hecm_draw", default=inp.get("hecm_draw", 0.0), drawer_name="home_equity", help_text="If drawing from a HECM, enter the expected monthly draw; this is treated as funding, not earned income.")
            else:
                inp["hecm_draw"] = 0.0
            if inp.get("expect_heloc"):
                inp["heloc_draw"] = currency_input("HELOC monthly draw (optional)", "heloc_draw", default=inp.get("heloc_draw", 0.0), drawer_name="home_equity", help_text="Enter a planned monthly draw from a HELOC if you intend to use it to help cover care costs.")
            else:
                inp["heloc_draw"] = 0.0
    ...
    # -------- Step 4 --------
        ...
        with cta1:
            if st.button("← Back to finances", key="back_to_step3_from_step4"):
                st.session_state.step = 3
                st.rerun()
        with cta2:
