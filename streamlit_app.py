...
def load_spec():
    ...
    # Monthly ceilings approximation (MAPR / 12)
...
# Currency input with $0.00 placeholder and on-blur mark_touched
...
def main():
    ...
    # -------- Step 3 --------
        ...
        try:
            res_tmp = compute(inp, spec)
            need_equity_nudge = bool(res_tmp.get("gap", 0.0) > 0.0) or any(
                (inp.get(f"care_type_{t}") in ["Assisted Living (or Adult Family Home)", "Memory Care"]) for t in ("a","b")
            )
        except Exception:
            need_equity_nudge = False
        if need_equity_nudge:
            ensure_touched_store()
            st.session_state.open_drawers["home_equity"] = True
